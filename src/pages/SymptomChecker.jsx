...
const MOCK_RESPONSES = {
  ...
  chest: { possibleConditions: ["Angina", "Acid reflux", "Musculoskeletal strain", "Anxiety"], recommendedSpecialist: "Cardiologist", urgency: "high", urgencyReason: "Chest symptoms require prompt cardiac evaluation", generalAdvice: "Chest pain should always be evaluated by a medical professional. Avoid strenuous activity until assessed.", redFlags: ["Pain radiating to arm or jaw", "Sweating with chest pain", "Shortness of breath", "Sudden severe pain"] },
  skin: { possibleConditions: ["Eczema", "Contact dermatitis", "Psoriasis", "Fungal infection"], recommendedSpecialist: "Dermatologist", urgency: "low", urgencyReason: "Skin conditions are generally non-urgent unless rapidly spreading", generalAdvice: "Avoid scratching the affected area. Keep skin moisturized and avoid known irritants.", redFlags: ["Rapidly spreading rash", "Fever with skin changes", "Blistering", "Signs of infection"] },
  head: { possibleConditions: ["Tension headache", "Migraine", "Sinusitis", "Hypertension"], recommendedSpecialist: "Neurologist", urgency: "medium", urgencyReason: "Persistent headaches warrant professional evaluation", generalAdvice: "Rest in a quiet, dark room. Stay hydrated and avoid triggers like bright lights or loud sounds.", redFlags: ["Sudden severe headache", "Headache with stiff neck", "Confusion or vision changes"] },
  stomach: { possibleConditions: ["Gastritis", "Irritable bowel syndrome", "Food intolerance", "Peptic ulcer"], recommendedSpecialist: "General Physician", urgency: "low", urgencyReason: "Digestive symptoms are common and usually manageable", generalAdvice: "Eat small, frequent meals and avoid spicy or fatty foods. Stay hydrated.", redFlags: ["Blood in stool", "Severe abdominal pain", "Unexplained weight loss"] },
  breathing: { possibleConditions: ["Asthma", "Bronchitis", "Respiratory infection", "Allergic reaction"], recommendedSpecialist: "Pulmonologist", urgency: "high", urgencyReason: "Breathing difficulties require prompt medical attention", generalAdvice: "Avoid known respiratory irritants like smoke or dust. Rest and avoid exertion.", redFlags: ["Severe shortness of breath", "Blue lips or fingertips", "Inability to speak in full sentences"] },
  joint: { possibleConditions: ["Arthritis", "Tendinitis", "Ligament strain", "Gout"], recommendedSpecialist: "Orthopedic", urgency: "low", urgencyReason: "Joint pain is usually manageable with rest and medication", generalAdvice: "Rest the affected joint and apply ice to reduce swelling. Avoid activities that worsen the pain.", redFlags: ["Severe swelling with fever", "Inability to bear weight", "Joint deformity"] },
  mental: { possibleConditions: ["Anxiety disorder", "Depression", "Stress-related symptoms", "Sleep disorder"], recommendedSpecialist: "Psychiatrist", urgency: "medium", urgencyReason: "Mental health symptoms benefit from early professional support", generalAdvice: "Prioritize sleep and regular exercise. Practice relaxation techniques like deep breathing.", redFlags: ["Thoughts of self-harm", "Severe inability to function", "Complete social withdrawal"] }
};

function getMockResult(symptoms) {
  const s = symptoms.toLowerCase();
  if (s.includes("chest") || s.includes("heart")) return MOCK_RESPONSES.chest;
  if (s.includes("skin") || s.includes("rash") || s.includes("itch")) return MOCK_RESPONSES.skin;
  if (s.includes("head") || s.includes("migraine") || s.includes("dizzy")) return MOCK_RESPONSES.head;
  if (s.includes("stomach") || s.includes("nausea") || s.includes("abdomen")) return MOCK_RESPONSES.stomach;
  if (s.includes("breath") || s.includes("cough") || s.includes("asthma")) return MOCK_RESPONSES.breathing;
  if (s.includes("joint") || s.includes("knee") || s.includes("back") || s.includes("muscle")) return MOCK_RESPONSES.joint;
  if (s.includes("anxiety") || s.includes("depress") || s.includes("stress") || s.includes("sleep")) return MOCK_RESPONSES.mental;
      ...
      <div style={{ display: 'flex', minHeight: '100vh', background: '#f9fafb' }}>
        ...
        <main style={{ marginLeft: 250, flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          <header style={{ background: 'white', borderBottom: '1px solid #e5e7eb', padding: '24px 32px', flexShrink: 0 }}>
            ...
            <h1 style={{ fontSize: 22, fontWeight: 800, color: '#111827', letterSpacing: '-0.01em' }}>Symptom Checker</h1>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
              ...
              <section>
                <div className="card" style={{ marginBottom: 16 }}>
                  <p style={{ fontSize: 14, fontWeight: 700, color: '#111827', marginBottom: 20, paddingBottom: 14, borderBottom: '1px solid #f3f4f6' }}>Tell us about your symptoms</p>
                ...
                <div style={{ background: '#fffbeb', borderRadius: 10, padding: 16, border: '1px solid #fde68a', borderLeft: '4px solid #f59e0b' }}>
                  ...
                  <p style={{ fontSize: 12, color: '#78350f', lineHeight: 1.6 }}>This tool is for informational purposes only and does not replace professional medical advice.</p>
                </div>
              </section>
