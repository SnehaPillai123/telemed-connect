...
export default function LabReports() {
  ...
  const handleUpload = async (file) => {
    setError("");
    if (!file) return;
    if (!reportName.trim()) { setError("Please enter a report name first"); return; }
    if (file.size > 10 * 1024 * 1024) { setError("File too large. Maximum size is 10MB."); return; }

    const allowed = ["application/pdf","image/jpeg","image/jpg","image/png"];
    if (!allowed.includes(file.type)) { setError("Only PDF, JPG, and PNG files are allowed."); return; }

    // Check storage is initialized
    if (!storage) { toast.error("Firebase Storage not configured. Add storage export to firebase/config.js"); return; }
    ...
    try {
      ...
      const storageRef = ref(storage, `labReports/${user.uid}/${Date.now()}_${safeName}`);
      ...
      uploadTask.on("state_changed",
        ...
        async () => {
          ...
          const newDoc = await addDoc(collection(db, "labReports"), {
            patientId: user.uid,
  ...
  const typeColor = (t) => {
    const m = {"Blood Test":"#dc2626","X-Ray":"#7c3aed","MRI Scan":"#2563eb","CT Scan":"#0d9488","ECG":"#d97706","Ultrasound":"#16a34a"};
    return m[t]||"#6b7280";
  };
  ...
  return (
    <Layout title="Lab Reports" subtitle="Patient Portal">
      <style>{`
        ...
        @media(max-width:599px){.lab-form-grid{grid-template-columns:1fr!important}.lab-stats-grid{grid-template-columns:repeat(2,1fr)!important}}
      `}</style>

      {/* Stats */}
      <div className="lab-stats-grid" style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12, marginBottom:20 }}>
        {[
          { l:"Total Reports", v:reports.length, c:"#0d9488", bg:"#f0fdfa" },
          { l:"This Month", v:reports.filter(r=>new Date((r.createdAt?.seconds||0)*1000).getMonth()===new Date().getMonth()).length, c:"#2563eb", bg:"#eff6ff" },
      ...
      {/* Upload — only for patients */}
      {role !== "doctor" && (
        <div style={{ background:"white", borderRadius:14, border:"1px solid #e5e7eb", padding:"20px", marginBottom:20 }}>
          <p style={{ fontSize:15, fontWeight:700, color:"#111827", marginBottom:14 }}>Upload New Report</p>
          ...
          <div className="lab-form-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:14 }}>
            <div>
          ...
          <div
            ...
            style={{ border:"2px dashed "+(dragOver?"#0d9488":"#e5e7eb"), borderRadius:12, padding:"28px 20px", textAlign:"center", cursor:uploading?"not-allowed":"pointer", background:dragOver?"#f0fdfa":"#fafafa", transition:"all 0.2s" }}>
            <div style={{ width:48, height:48, borderRadius:12, background:dragOver?"#f0fdfa":"#f3f4f6", border:"1px solid "+(dragOver?"#ccfbf1":"#e5e7eb"), display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 10px" }}>
            ...
            {uploading ? (
              <p style={{ fontSize:14, color:"#0d9488", fontWeight:600 }}>Uploading... {progress}%</p>
            ) : (
              <>
                ...
                <p style={{ fontSize:12, color:"#9ca3af" }}>PDF, JPG, PNG · Max 10MB</p>
              </>
            )}
          ...
          {uploading && (
            <div style={{ marginTop:12 }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:5 }}>
                <p style={{ fontSize:13, color:"#374151" }}>Uploading report...</p>
      ...
      <p style={{ fontSize:14, fontWeight:700, color:"#111827", marginBottom:12 }}>
        {role==="doctor" ? "Patient Reports" : "Your Reports"}
      </p>
      ...
      {!loading && reports.length === 0 && (
        <div style={{ background:"white", borderRadius:12, border:"1px solid #e5e7eb", padding:"40px", textAlign:"center" }}>
          <svg width="44" height="44" fill="none" viewBox="0 0 24 24" style={{ margin:"0 auto 12px", display:"block" }}>
          ...
          <p style={{ fontSize:13, color:"#9ca3af" }}>{role==="patient"?"Upload your first lab report above.":"No reports shared by patients yet."}</p>
        </div>
      )}

      {reports.map((r,i) => (
        <div key={r.id} className="fade-in" style={{ background:"white", borderRadius:12, border:"1px solid #e5e7eb", padding:"12px 16px", marginBottom:10, display:"flex", gap:12, alignItems:"center", animationDelay:i*0.04+"s", opacity:0 }}>
          <div style={{ width:42, height:42, borderRadius:9, background:typeColor(r.reportType)+"18", border:"1px solid "+typeColor(r.reportType)+"30", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
            <svg width="18" height="18" fill="none" viewBox="0 0 24 24">
              <path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" stroke={typeColor(r.reportType)} strokeWidth="1.8" strokeLinecap="round"/>
            </svg>
          </div>
          <div style={{ flex:1, minWidth:0 }}>
            ...
            <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"center" }}>
              <span style={{ fontSize:11, fontWeight:600, color:typeColor(r.reportType), background:typeColor(r.reportType)+"18", padding:"2px 8px", borderRadius:20 }}>{r.reportType}</span>
              <span style={{ fontSize:11, color:"#9ca3af" }}>{formatSize(r.fileSize)}</span>
              <span style={{ fontSize:11, color:"#9ca3af" }}>{r.createdAt?.toDate?.()?.toLocaleDateString("en-IN")||"Recently"}</span>
            </div>
          </div>
          <a href={r.fileURL} target="_blank" rel="noreferrer"
            style={{ padding:"7px 14px", background:"#0d9488", color:"white", borderRadius:8, fontSize:12, fontWeight:600, textDecoration:"none", flexShrink:0, display:"flex", alignItems:"center", gap:5 }}>
