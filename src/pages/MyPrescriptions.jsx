    ...
    <>
      <style>{`
        ...
        * { font-family: 'Inter', sans-serif; box-sizing: border-box; margin: 0; padding: 0; }
        .rx-card { background: white; border-radius: 10px; border: 1px solid #e5e7eb; overflow: hidden; transition: all 0.2s; }
          ...
          <div style={{ maxWidth: 1000, margin: '0 auto', padding: '32px 40px' }}>
            ...
            {!loading && prescriptions.length === 0 && (
              <div style={{ background: 'white', borderRadius: 10, border: '1px solid #e5e7eb', padding: '60px', textAlign: 'center' }}>
                ...
                <p style={{ fontSize: 13, color: '#9ca3af' }}>Your prescriptions from doctors will appear here after completed consultations.</p>
              </div>
            )}
              ...
              {prescriptions.map((rx, idx) => (
                <article key={rx.id} className="rx-card fade-in" style={{ animationDelay: `${idx * 0.06}s`, opacity: 0 }} role="listitem">
                  ...
                  <div className="rx-header" onClick={() => setExpanded(expanded === rx.id ? null : rx.id)}
                    aria-expanded={expanded === rx.id} aria-controls={`rx-body-${rx.id}`}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <svg width="16" height="16" fill="none" viewBox="0 0 24 24" style={{ transition: 'transform 0.2s', transform: expanded === rx.id ? 'rotate(180deg)' : 'none' }} aria-hidden="true">
