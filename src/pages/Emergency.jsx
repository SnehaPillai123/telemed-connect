...
export default function Emergency() {
  ...
  const activateSOS = () => {
    setSosActive(true); setCountdown(5); setLocationStatus("getting");
    ...
    <>
      <style>{`
        ...
        * { font-family: 'Inter', sans-serif; box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .hospital-card { background: white; border-radius: 10px; border: 1px solid #e5e7eb; padding: 16px 18px; display: flex; justify-content: space-between; align-items: center; transition: all 0.2s; margin-bottom: 10px; }
        .hospital-card:hover { border-color: #0d9488; }
      ...
      <div style={{ display: 'flex', minHeight: '100vh', background: '#f9fafb' }}>
        ...
        <main style={{ marginLeft: 250, flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          <header style={{ background: 'white', borderBottom: '1px solid #e5e7eb', padding: '24px 32px', flexShrink: 0 }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 50, padding: '4px 14px', marginBottom: 10 }}>
              <div style={{ width: 7, height: 7, borderRadius: '50%', background: '#ef4444' }}/>
              <span style={{ fontSize: 11, fontWeight: 700, color: '#dc2626', letterSpacing: '0.06em' }}>EMERGENCY SERVICES</span>
            </div>
            <h1 style={{ fontSize: 22, fontWeight: 800, color: '#111827', letterSpacing: '-0.01em' }}>Emergency SOS</h1>
            <p style={{ fontSize: 13, color: '#6b7280', marginTop: 4 }}>In a medical emergency, hold the SOS button. It will share your location and alert emergency services.</p>
          </header>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 28, alignItems: 'start' }}>
              ...
              <section>
                {!sosTriggered ? (
                  <div style={{ background: sosActive ? '#fef2f2' : 'white', borderRadius: 16, border: `2px solid ${sosActive ? '#fecaca' : '#e5e7eb'}`, padding: '40px 28px', textAlign: 'center', marginBottom: 16, transition: 'all 0.3s' }}>
                    {sosActive ? (
                      <div>
                        ...
                        <div style={{ fontSize: 80, fontWeight: 900, color: '#dc2626', lineHeight: 1, marginBottom: 20 }}>{countdown}</div>
                        <p style={{ fontSize: 14, color: '#991b1b', marginBottom: 20 }}>
                  ...
                  <div className="fade-up" style={{ background: '#fef2f2', borderRadius: 16, border: '2px solid #fecaca', padding: '36px 28px', textAlign: 'center', marginBottom: 16 }}>
                    ...
                    {location && <p style={{ fontSize: 13, color: '#991b1b', marginBottom: 20 }}>Your location: {location.lat.toFixed(4)}, {location.lng.toFixed(4)}</p>}
                ...
                <div style={{ background: 'white', borderRadius: 10, border: '1px solid #e5e7eb', padding: '18px 20px' }}>
                  <p style={{ fontSize: 13, fontWeight: 700, color: '#111827', marginBottom: 12 }}>What happens when you activate SOS:</p>
                  {['Your GPS location is captured instantly', 'Nearby hospitals are alerted with your location', 'Emergency contacts receive notification', 'Emergency helpline numbers are displayed'].map((s, i) => (
                  ...
                  {NEARBY_HOSPITALS.map((h, i) => (
                    <div key={i} className="hospital-card">
                      <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                        <div style={{ width: 38, height: 38, borderRadius: 8, background: '#fef2f2', border: '1px solid #fecaca', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <svg width="18" height="18" fill="none" viewBox="0 0 24 24"><path d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" stroke="#dc2626" strokeWidth="1.5" strokeLinecap="round"/></svg>
                        </div>
                        <div>
