    ...
    <>
      <style>{`
        ...
        * { font-family: 'Inter', sans-serif; box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .med-card { background: white; border-radius: 12px; border: 1px solid #e5e7eb; padding: 16px 20px; margin-bottom: 10px; transition: all 0.2s; }
        .med-card:hover { border-color: #0d9488; }
        .day-btn { width: 36px; height: 36px; border-radius: 8px; border: 1.5px solid #e5e7eb; background: white; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.15s; font-family: Inter, sans-serif; }
        .day-btn.taken { background: #0d9488; border-color: #0d9488; }
        .day-btn.today-btn { border-color: #0d9488; }
      ...
      <div className="layout">
        ...
        <main className="main">
          <header style={{ background: 'white', borderBottom: '1px solid #e5e7eb', padding: '24px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button onClick={() => setShowAdd(!showAdd)} style={{ padding: '9px 18px', background: '#0d9488', color: 'white', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>
              + Add Medication
            </button>
          </header>
          ...
          <div style={{ padding: '28px 32px' }}>
            ...
            {showAdd && (
              <div className="fade-in" style={{ background: 'white', borderRadius: 12, border: '1px solid #e5e7eb', padding: '24px', marginBottom: 24 }}>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  <button onClick={addMed} style={{ padding: '9px 20px', background: '#0d9488', color: 'white', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>Add</button>
                  <button onClick={() => setShowAdd(false)} style={{ padding: '9px 20px', background: 'white', color: '#374151', border: '1.5px solid #e5e7eb', borderRadius: 8, fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>Cancel</button>
                </div>
              </div>
            )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 260px', gap: 24, alignItems: 'start' }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', marginBottom: 12, paddingLeft: 4 }}>
                ...
                {meds.map((med, mi) => (
                  <div key={med.id} className="med-card fade-in" style={{ animationDelay: `${mi*0.06}s`, opacity: 0, display: 'flex', alignItems: 'center', gap: 16 }}>
                    <div style={{ width: 200, flexShrink: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      {DAYS.map((d, i) => (
                        <button key={i}
                          className={`day-btn ${med.taken[i] ? 'taken' : ''} ${i === todayIdx ? 'today-btn' : ''}`}
                          onClick={() => toggleTaken(med.id, i)}
                          title={`${d} - ${med.taken[i] ? 'Taken' : 'Not taken'}`}>
                          {med.taken[i] ? (
                            <svg width="14" height="14" fill="none" viewBox="0 0 24 24">
                              <path d="M5 13l4 4L19 7" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
                            </svg>
                          ) : (
                            <span style={{ fontSize: 10, color: i === todayIdx ? '#0d9488' : '#d1d5db', fontWeight: 600 }}>{d[0]}</span>
                          )}
                        </button>
                      ))}
                    </div>
                ...
                <div style={{ background: 'white', borderRadius: 12, border: '1px solid #e5e7eb', padding: '20px', textAlign: 'center' }}>
                  ...
                  <div style={{ position: 'relative', width: 100, height: 100, margin: '0 auto 12px' }}>
                    ...
                    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
                      <p style={{ fontSize: 20, fontWeight: 800, color: '#0d9488', lineHeight: 1 }}>{adherence}%</p>
                      <p style={{ fontSize: 9, color: '#9ca3af', fontWeight: 500 }}>taken</p>
                    </div>
                  </div>
                  <p style={{ fontSize: 13, color: '#6b7280' }}>{todayTaken} of {meds.length} taken today</p>
                </div>

                <div style={{ background: 'white', borderRadius: 12, border: '1px solid #e5e7eb', padding: '20px' }}>
                  <p style={{ fontSize: 13, fontWeight: 700, color: '#111827', marginBottom: 12 }}>Today's Schedule</p>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {meds.map((med, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', background: med.taken[todayIdx] ? '#f0fdfa' : '#f9fafb', borderRadius: 8, border: `1px solid ${med.taken[todayIdx] ? '#ccfbf1' : '#f3f4f6'}` }}>
                        <div style={{ width: 7, height: 7, borderRadius: '50%', background: med.taken[todayIdx] ? '#0d9488' : '#d1d5db', flexShrink: 0 }}/>
                        <p style={{ fontSize: 12, fontWeight: 600, color: '#111827', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{med.name}</p>
